class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        while l in range(len(nums)) and r in range(len(nums)) and l <= r:
            while l in range(len(nums)) and nums[l] != val:
                l += 1
            r = l
            while r in range(len(nums)) and nums[r] == val:
                r += 1
            if l in range(len(nums)) and r in range(len(nums)):
                temp = nums[r]
                nums[r] = nums[l]
                nums[l] = temp
